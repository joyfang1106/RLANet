...
parser.add_argument('-a', '--arch', metavar='ARCH', default='rla_resnet50',
                    choices=model_names,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained_conv1:
        pretrain_model = None
        arch = args.arch
        if arch.find('resnet50') > -1: # in ['rla_resnet50', 'rla_resnet50_eca']:
            pretrain_model = torchmodels.resnet50(pretrained=True)
        elif arch.find('resnet101') > -1: # in ['rla_resnet101', 'rla_resnet101_eca']:
            pretrain_model = torchmodels.resnet101(pretrained=True)
        elif arch.find('resnet152') > -1: # in ['rla_resnet152', 'rla_resnet152_eca']:
            pretrain_model = torchmodels.resnet152(pretrained=True)
        elif arch.find('resnext50_32x4d') > -1: # in ['rla_resnext50_32x4d', 'rla_resnext50_32x4d_eca']:
            pretrain_model = torchmodels.resnext50_32x4d(pretrained=True)
        elif arch.find('resnext101_32x4d') > -1: # in ['rla_resnext101_32x4d', 'rla_resnext101_32x4d_eca']:
            pretrain_model = torchmodels.resnext101_32x8d(pretrained=True)
            # print("[!] resnext101_32x4d is not available in torchvision.models, proceed WITHOUT using pretrained conv1")
